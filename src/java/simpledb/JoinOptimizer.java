    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        for (int i = 0; i < joins.size(); i++) {
            ...
            for (Set<LogicalJoinNode> s : subsets) {
                planCache.addPlan(s, Double.MAX_VALUE, Integer.MAX_VALUE, null);
                for (LogicalJoinNode node : s) {
                    CostCard ret = computeCostAndCardOfSubplan(
                            stats
                            , filterSelectivities
                            , node
                            , s
                            ...
                            , planCache);
                    if (ret != null) {
                        planCache.addPlan(s, ret.cost, ret.card, ret.plan);
                    }
                }
            }
        }
        Set<LogicalJoinNode> s = new HashSet<>(joins);
        Vector<LogicalJoinNode> result = planCache.getOrder(s);
        if (explain) {
